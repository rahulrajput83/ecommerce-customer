import Head from 'next/head'
import Link from 'next/link';
import { useRouter } from 'next/router';
import { useEffect, useState } from 'react';
import Navbar from '../../components/Navbar'
import moment from 'moment/moment';
import { getRequest } from '@/Functions/Requests';
import ErrorComponent from '@/components/ErrorComponent'
import { Logout } from '@/Functions/Logout';
import Card from '@/components/Card';
const CryptoJS = require('crypto-js')

export default function product() {
    const router = useRouter();
    const [data, setData] = useState();
    const [loading, setLoading] = useState(true)
    const [getError, setGetError] = useState(false)

    useEffect(() => {
        const getProduct = async () => {
            if (router.isReady) {
                try {
                    setLoading(true)
                    const { id } = router.query;
                    const res = await getRequest(`/api/sellerDetail/${id}`)
                    if (res.message && res.message === 'Unauthorized') {
                        router.push(Logout());
                    }
                    else if (res.message && res.message === 'Error, please try again') {
                        setGetError(true);
                        setTimeout(() => {
                            setGetError(false)
                        }, 6000)
                    }
                    else {
                        const user = res.user;
                        console.log(user)
                        let bytesFullName = CryptoJS.AES.decrypt(user.fullName, process.env.JWT);
                        let decryptFullName = bytesFullName.toString(CryptoJS.enc.Utf8);
                       
                        
                        user.fullName = decryptFullName;
                        
                        setData(user)
                        setLoading(false)
                    }
                } catch (error) {
                    console.log(error)
                    setGetError(true)
                    setTimeout(() => {
                        setGetError(false)
                    }, 6000)
                }
            }
        }
        getProduct();
    }, [router.isReady])


    return (
        <>
            <Head>
                <title>{data && data.displayName ? data.displayName : 'Seller'}</title>
                <meta name="description" content="Generated by create next app" />
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                <link rel="icon" href="favicon.ico" />
            </Head>

            <main className='w-100 flex flex-col'>
                <Navbar />
                {getError && <ErrorComponent />}
                <div className='w-full flex flex-col gap-2 px-2 md:px-10 pb-10 mt-20 md:mt-20'>
                    {!loading
                        ?
                        <div className='w-full grid grid-cols-1 md:grid-cols-4 gap-6'>
                            <div className='md:z-30 w-full relative flex flex-col'>
                                <div className='shadow-md bg-white rounded p-4 md:sticky top-16 w-full flex flex-col justify-center items-center'>
                                    <img src='/7309681.jpg' alt='' className='rounded-full h-24 w-24' />
                                    <span className='mt-4'>{data.displayName}</span>
                                    <span className='text-xs'>{data.address}</span>
                                </div>
                            </div>
                            <div className='w-full md:col-span-3 flex flex-col gap-2'>
                                <span>Products</span>
                                <div className='w-full grid grid-cols-2 md:grid-cols-3 gap-4'>
                                    {data && data.products && data.products.length > 0 ?
                                        data.products.map((e, i) => {
                                            return (
                                                <Card data={e} key={i} />
                                            )
                                        })
                                        :
                                        <span className='w-full text-sm text-center col-span-2 md:col-span-3'>No Product Found</span>}
                                </div>
                            </div>
                        </div>
                        :
                        <div className='w-full animate-pulse grid gap-6 grid-cols-1 md:grid-cols-12'>
                            <div className='md:col-span-3 gap-1 p-2 md:p-4 bg-slate-200 h-52 rounded shadow-lg w-full flex flex-col justify-start items-start'>
                            </div>
                            <div className='md:col-span-3 gap-1 p-2 md:p-4 bg-slate-200 h-52 rounded shadow-lg w-full flex flex-col justify-start items-start'>
                            </div>
                            <div className='md:col-span-3 gap-1 p-2 md:p-4 bg-slate-200 h-52 rounded shadow-lg w-full flex flex-col justify-start items-start'>
                            </div>
                            <div className='md:col-span-3 gap-1 p-2 md:p-4 bg-slate-200 h-52 rounded shadow-lg w-full flex flex-col justify-start items-start'>
                            </div>
                        </div>
                    }
                </div>
            </main>
        </>
    )
}